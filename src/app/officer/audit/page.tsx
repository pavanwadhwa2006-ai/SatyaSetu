...
import { fetchBidders } from "@/lib/mock-api";
import { Bidder } from "@/types";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { ScrollText, ArrowRight, Building2, Loader2 } from "lucide-react";
import { PreviousButton } from "@/components/shared/previous-button";

export default function OfficerAuditPage() {
  const [bidderList, setBidderList] = useState<Bidder[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let isMounted = true;
    fetchBidders().then((data) => {
      if (isMounted) {
        setBidderList(data);
        setLoading(false);
      }
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="flex items-center justify-center py-12 text-muted-foreground gap-2">
            ...
            <span className="text-sm">Loading vendors from database...</span>
          </div>
        ) : (
          bidderList.map((bidder) => (
            <Card key={bidder.id}>
              <CardContent className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                ...
                <Link href={`/officer/tenders/${(bidder as any).tenderId || 'tenders'}/bidders/${bidder.id}/audit`} className="w-full sm:w-auto">
