                ...
                <TableRow key={doc.id} className={doc.verificationStatus === "REVIEW" ? "bg-amber-50/50" : doc.verificationStatus === "FAIL" ? "bg-red-50/30" : ""}>
                  <TableCell>
                    <div className="min-w-0">
                      <p className="text-xs sm:text-sm font-medium leading-snug">{doc.typeName}</p>
                      <p className="text-[11px] text-muted-foreground">{doc.fileName} · {doc.pageCount} page{doc.pageCount > 1 ? "s" : ""}</p>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="text-right">
                    <Button size="sm" variant="ghost" className="h-7 text-xs text-[#1e3a5f] hover:bg-[#1e3a5f]/10" onClick={() => setSelectedDoc(doc)}>
                      View Details
                    </Button>
                  </TableCell>
                </TableRow>
            ...
            return (
              <>
                <DialogHeader className="text-left">
                  <DialogTitle className="text-sm sm:text-base font-semibold leading-snug">{selectedDoc.typeName}</DialogTitle>
                ...
                <div className="space-y-2 mt-2">
                  ...
                  <div className="rounded-md border p-3 space-y-2 bg-card">
                    {selectedDoc.extractedFields.map((field) => (
                      <div key={field.fieldName} className="flex justify-between items-start text-xs sm:text-sm gap-2">
                        <span className="text-muted-foreground shrink-0">{field.fieldName}</span>
                        <span className="font-medium text-right break-words">{field.extractedValue}</span>
                      </div>
                    ))}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="rounded-md border p-3 space-y-2 bg-card">
                        {Object.entries(ver.verificationData).map(([key, value]) => {
                          ...
                          return (
                            <div key={key} className={`flex justify-between items-start text-xs sm:text-sm gap-2 ${mismatch ? "bg-red-50 -mx-2 px-2 py-1 rounded" : ""}`}>
                              <span className="text-muted-foreground shrink-0">{key}</span>
                              <span className={`font-medium text-right break-words ${mismatch ? "text-red-700 font-semibold" : ""}`}>{value}</span>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    {ver.discrepancies.length > 0 && (
                      <div className="rounded-md bg-red-50 border border-red-200 p-3">
                        ...
                        {ver.discrepancies.map((d, i) => (
                          <p key={i} className="text-xs text-red-700 leading-relaxed">{d}</p>
                        ))}
                      </div>
                    )}
                  </>
                ...
                {/* Result */}
                <div className="flex items-center justify-between rounded-md border p-3 bg-muted/30 mt-2">
                  <span className="text-xs sm:text-sm font-medium">Verification Result</span>
