...
export default function OfficerTendersPage() {
  ...
  const loadTenders = async () => {
    ...
    try {
      ...
      console.error("Failed to load officer tenders:", err);
      setError(err.message || "Failed to fetch tenders from backend.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              Select an active GeM tender to evaluate submitted bidder packages
            </p>
          </div>
        </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 sm:gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1.5">
                      <span className="text-xs font-mono text-muted-foreground font-semibold bg-slate-100 px-2 py-0.5 rounded">
                  ...
                  <Link
                    href={`/officer/tenders/${encodeURIComponent(tender.tender_number || tender.id)}`}
