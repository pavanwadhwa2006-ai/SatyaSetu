...
## Decision 012 — Document Intelligence & Deterministic Fact Extraction
In Phase 7, the Document Intelligence layer was built to parse uploaded bidder PDFs from Supabase Storage using `pypdf`, classify documents into 12 canonical procurement types, and extract structured domain facts with numeric normalizations, 1-indexed source page numbers, and verbatim quotations. Strict phase boundaries are maintained: the engine solely extracts factual statements and never calculates compliance verdicts (`PASS/FAIL/COMPLIANT/NON_COMPLIANT`). Ground truth benchmark verdicts are never leaked or used as extraction inputs.

## Decision 013 — Frontend Real Backend Data Integration Architecture
In Phase 7.1, the entire Next.js frontend was converted into a pure UI, presentation, and user interaction layer driven exclusively by the real FastAPI backend and Supabase database. All legacy static mock datasets (e.g. `GEM-DEMO-2026-001`, hardcoded tenderRows, mock statistics) were completely removed from application views. Dynamic analytics (`GET /api/dashboard/stats`), live GeM tenders (`GET /api/tenders`), vendor listings (`GET /api/vendors`), Tender Intelligence (`GET /api/tender-intelligence/*`), Bid Submissions (`POST /api/bid-submissions`, `GET /api/bid-submissions`), Document Intelligence (`POST /api/documents/{id}/process`, `GET /api/documents/{id}/facts`), and Ground Truth benchmarks (`GET /api/ground-truth/*`) are connected through a strongly-typed, centralized API client (`src/lib/api-client.ts`) with clear loading and error feedback states without silent fake fallbacks.
